...
import 'package:beauity_saloon/view/CashInOutListController.dart';
...
class CashInOutList extends StatefulWidget {
  const CashInOutList({Key? key}) : super(key: key);

  @override
  State<CashInOutList> createState() => _CashInOutListState();
}

class _CashInOutListState extends State<CashInOutList> {
  late CashInOutListController controller;
  ...
  void initState() {
    super.initState();
    controller = Get.put(CashInOutListController());
    controller.getCashRegister();
  }

  /// For search this below list is used...
  // List<PosSettlement> filterList(String query) {
  //   return controller.getPosSettlementList.where((item) {
  //     return item.settlementNo.toString().toLowerCase().contains(query.toLowerCase());
  //   }).toList();
  // }

  @override
  Widget build(BuildContext context) {
    return GetBuilder<CashInOutListController>(builder: (logic) {
            ...
            appBar: AppBar(
                ...
                actions: [
                  ...
                  IconButton(
                      ...
                      color: MyColors.w,
                      onPressed: () {
                        Get.toNamed(AppRoutes.addCashInOut);
                      },
                      iconSize: 30),
                                ...
                                Expanded(
                                  child: InkWell(
                                    onTap: () {
                                      ...
                                      /// call the api of header search below.
                                    },
                                    child: Container(
  ...
  list() {
    // if (controller.getPosSettlementList.value.isNotEmpty && controller.getPosSettlementList.value != null ){
    return ListView.builder(
        physics: const NeverScrollableScrollPhysics(),
        shrinkWrap: true,
        itemCount: 2,
        // search.isEmpty? controller.getPosSettlementList.value.length : filterList(search).length,
        itemBuilder: (context, index) {
          // final items = search.isEmpty? controller.getPosSettlementList.value[index] : filterList(search)[index];
                  ...
                  child: Table(
                    ...
                    children: [
                      TableRow(children: [
                        ...
                        Text(
                          // formatDate(controller.getPosSettlementList
                          //     .value[index]
                          //     .settlementDate) ??
                          '02/11/2023',
                          style: const TextStyle(
